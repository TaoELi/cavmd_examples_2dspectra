...
if [ ! -d $NVTFOLDER ]; then
    ...
    sed -i "s/h2o-dipole/h2o-dipole_nvt_"$E0"_"$traj"/" input.xml
...
if [ -f $CHECKFILE ] && grep -q $NVTPATTERN "$CHECKFILE"; then
    ...
    rm -rf /tmp/ipi_h2o-dipole_nvt_"$E0"_"$traj"
    if [ ! -f $CHECKFILE ] ; then
    ...
    i-pi-driver -u -h h2o-dipole_nvt_"$E0"_"$traj" -m water_dip_pol -o 1 &> out
    sleep 10s
