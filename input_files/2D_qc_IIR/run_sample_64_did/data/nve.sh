...
NONEQIRFOLDER=$HOMEFOLDER/result/noneq-qc-raman
if [ ! -d $NONEQIRFOLDER ]; then
    mkdir $NONEQIRFOLDER
fi

python ./noneqm-response.py input.xml -e 0.1 -t 1000 -b out,pol -p 0,0
mv $NVEFOLDER/out_pol_neq_2d.dat $NONEQIRFOLDER/out_pol_neq_2d.dat_$traj
sleep 1s
