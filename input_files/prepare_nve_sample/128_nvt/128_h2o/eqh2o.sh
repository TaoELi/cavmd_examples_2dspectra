...
ORIGINFOLDER=$TOTALFOLDER/128_h2o
...
if [ -f "$CHECKPOINTFOLDER/init_0.checkpoint" ]; then
    ...
    if [ ! -d "$NVTFOLDER" ]; then
        ...
        sed -i "s/water_280/water_280_"$nmol"_nvt_128/" input_eq.xml
        sed -i "s/water_280/water_280_"$nmol"_nvt_128/" in.lmp

    fi
    
    cd $NVTFOLDER
    echo 'NVT checkpoint file does not exist, run NVT equilibrate 100 ps'
    rm -rf /tmp/ipi_water_280_"$nmol"_nvt_128
...
traj=1
    ...
    if [ -f "$NVECHECKPOINTFILE" ]; then
        ...
        if [ ! -d "$NVEFOLDER" ]; then
            ...
            sed -i "s/water_280/water_280_"$nmol"_nve_"$traj"_128/" input_traj.xml
            sed -i "s/water_280/water_280_"$nmol"_nve_"$traj"_128/" in.lmp
        ...
        rm -rf /tmp/ipi_water_280_"$nmol"_nve_"$traj"_128
