...
class NonEqmTraj(object):
    ...
    def prepare_for_run(self, sim, step, kick):
        ...
        row, col = np.shape(new_beads.p)
        nstep, nrow, ncol = np.shape(self.der)
        new_der = np.zeros((nstep, row, col))
        new_der[:,:,:ncol] = self.der
        self.new_der = new_der
        sim.syslist[0].beads.q = new_beads.q
        sim.syslist[0].beads.p = (
            new_beads.p + 0.5 * kick * self.epsilon * self.new_der[step]
        )
        sim.step = 0
    ...
    def run(self, sim, t_first=None, t_last=None):
        ...
        der = der.reshape(dera,derb,-1,9)[:,:,:,0:3]
        ...
        for step in range(t_first, t_last + 1):
            for kick in [-1, 0, 1]:
                ...
                sim.run()
                info("step = " + str(step) + " kick = " + str(kick))
                #############################################################################################
