...
class MD_Analysis:
    ...
    def cacl_ph_traj(self):
        print("Calculating the autocorrelation function...")
        self.dacf_x = self.auto_correlation_function_simple(self.traj[0,0,:])
        self.dacf_y = self.auto_correlation_function_simple(self.traj[1,1,:])
        self.dacf_tot = self.dacf_x + self.dacf_y
    ...
    def output_ph_autocorrelation(self):
        local_filename = "%s.phdac.txt" %self.xyz_filename
        if os.path.isfile(local_filename + "dd"):
            print("Have calculated dipole autocorrelation for %s, skipping..." %self.xyz_filename)
        else:
            self.cacl_ph_traj()
            # output data
            data = np.zeros((np.size(self.dacf_x), 8))
            data[:, 0] = self.dacf_time_fs
            data[:, 1] = self.dacf_x
            data[:, 2] = self.dacf_y
            data[:, 3] = self.dacf_tot
            data[:, 4] = self.dacf_x_freq
            data[:, 5] = smooth(self.dacf_x_sp)
            data[:, 6] = smooth(self.dacf_y_sp)
            data[:, 7] = smooth(self.dacf_tot_sp)
            comments = "# dacf_time_fs, dacf_x, dacf_y, dacf_tot, freq, sp_x, sp_y, sp_tot"
